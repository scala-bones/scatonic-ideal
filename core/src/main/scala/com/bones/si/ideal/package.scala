...
package object ideal {
  ...
  case class FixedLengthCharacterType(length: Int, charset: Charset)
      extends IdealDataType // NON-Varying type
  ...
  /**
    ...
    */
  case class IdealColumn(
    name: String,
    dataType: IdealDataType,
    nullable: Boolean,
  ...
  /**
    ...
    */
  case class IdealTable(
    name: String,
    primaryKeyColumns: List[IdealColumn],
    columns: List[IdealColumn],
    foreignKeys: List[IdealForeignKey],
  ...
  object IdealTable {
    def apply(
      name: String,
      primaryKey: IdealColumn,
      columns: List[IdealColumn],
      foreignKeys: List[IdealForeignKey] = List.empty,
      uniqueConstraints: List[UniqueConstraint] = List.empty,
      remark: Option[String] = None
    ): IdealTable =
      IdealTable(name, List(primaryKey), columns, foreignKeys, uniqueConstraints, remark)
    def empty(name: String, description: Option[String] = None): IdealTable =
      IdealTable(name, List.empty, List.empty, List.empty, List.empty, description)
  }
