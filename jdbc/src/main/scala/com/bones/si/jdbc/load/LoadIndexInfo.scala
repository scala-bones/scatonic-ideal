...
object LoadIndexInfo extends DefaultLoader[IndexInfo] {
  ...
  override protected def extractRow(rs: ResultSet): IndexInfo = {
    ...
    val indexType = IndexType
      .findByShort(indexTypeShort)
      .getOrElse(
        throw new MissingDataException(s"Could not find IndexType fro value ${indexTypeShort}")
      )
    val ascDesc = Option(rs.getString("ASC_OR_DESC"))
      .map(x =>
        AscDesc
          .findByString(x)
          .getOrElse(throw new MissingDataException(s"Could not find value for AscDesc: ${x}"))
      )
    IndexInfo(
