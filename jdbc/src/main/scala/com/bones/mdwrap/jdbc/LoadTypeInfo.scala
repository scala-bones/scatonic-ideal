...
import com.bones.mdwrap.{DataType, DatabaseQuery, Nullable, Searchable, TypeInfo}
...
object LoadTypeInfo {
  ...
  protected def extractRow(rs: ResultSet): TypeInfo = {
    ...
    val dataType = DataType.findByConstant(dataTypeInt).getOrElse(throw new IllegalStateException(s"Could not find data type by id: $dataTypeInt"))
    val nullableInt = req(rs.getInt("NULLABLE"))
    val nullable = Nullable.findById(nullableInt).getOrElse(throw new IllegalStateException(s"Could not find nullable by id: $nullableInt"))
    val searchableInt = rs.getInt("SEARCHABLE")
    val searchable = Searchable.findBySearchableId(searchableInt).getOrElse(throw new IllegalStateException(s"Could not find searchable by id: $searchableInt"))
    TypeInfo(
