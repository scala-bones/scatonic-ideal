...
object LoadColumn {
  ...
  def load(databaseQuery: DatabaseQuery,
           borrow: Borrow[Connection]): Try[List[Column]] = {
    val queryParams = databaseQueryToHierarchyQuery(databaseQuery)
    borrow.borrow(con =>
      Try {
        queryParams
          .flatMap(queryParam => {
            val resultSet = con.getMetaData.getColumns(queryParam._1.orNull,
                                                       queryParam._2.orNull,
                                                       queryParam._3.orNull,
                                                       null)
  ...
  private def extractRow(rs: ResultSet): Column = {
    ...
    val nullable = Nullable
      ...
      .getOrElse(
        throw new IllegalStateException(
          s"Unknown nullable str: ${nullableStr}"))
    Column(
